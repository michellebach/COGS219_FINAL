...
# generate trials
def generate_trials(subj_code, seed,num_repetitions=1):
    import os
    import random
...
order =  ['subj_id','seed','number_of_trials']
runtime_vars= get_runtime_vars({'subj_id':'RP01', 'seed':10, 'number_of_trials':[50, 150, 250]}, order)
print(runtime_vars)

# generate trials
generate_trials(runtime_vars['subj_id'],runtime_vars['seed'],runtime_vars['number_of_trials'])
#read in trials
trial_path = os.path.join(os.getcwd(),'trials',runtime_vars['subj_id']+'_trials.csv')
...
try:
    ...
    pass 
separator=","
data_file = open(os.path.join(os.getcwd(),'data',runtime_vars['subj_id']+'_data.csv'),'w')
header = separator.join(["subj_id", "seed", "number_of_trials", "neuron_index","total_peak","p1","p2","p3","p4","p5"])
...
#image_path = load_files(os.path.join(cur_dir,"stimuli","image"),'.jpg',fileType="image",win=win)
